...
class TestBreeds:
    ...
    def test_breeds_status_code(self, headers):
        ...
        logger.info(
            f"Breeds status code test passed with status {response.status_code}"
        )
    ...
    def test_specific_breed(self, headers):
        ...
        assert response.status_code == 200, "Response status code should be 200"
        assert (
            response.json()["id"] == breed_code
        ), f"No breed found with code {breed_code}"
    ...
    def test_search_breeds_with_query_and_image_attached(self, headers):
        ...
        response = requests.get(
            f"{self.BREEDS_URL}/search?q={query}&attach_breed=0&attach_image=1",
            headers=headers,
        )
        assert response.status_code == 200, "Response status code should be 200"
        data = response.json()
